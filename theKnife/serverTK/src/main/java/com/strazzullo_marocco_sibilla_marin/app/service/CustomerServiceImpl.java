...
public class CustomerServiceImpl extends UnicastRemoteObject implements CustomerService {
     ...
     */
    @Override
    public List<Location> searchLocations(SearchFilter filter) throws RemoteException {
        try {
            if (filter == null) {
                filter = new SearchFilter.Builder().build();
            }
            System.out.println("Executing location search using filter parameters: " + filter);
            return locationDAO.search(filter);
        } catch (SQLException e) {
            System.err.println("SQLException encountered in searchLocations service method: " + e.getMessage());
